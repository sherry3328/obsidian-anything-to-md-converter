...
- 新增 TeX -> Markdown（Pandoc）队列转换命令。
- 新增 Pandoc 路径设置与缺失时的自动安装提示。

## v0.7.0

- 新增“表格健康检查”：转换写入前自动清理 Markdown 表格中的全空行与全空列。
- 新增“按文件夹移除列规则”：支持 `文件夹路径 => 列名1,列名2` 的目录级列清理。
- 健康检查接入 MinerU 与 Mathpix 转换流程，并接入历史 HTML 表格批量转换。
- 转换提示新增表格清理统计（空行/列）。
